...
const QuotaManager = {
    ...
    loadQuotaData() {
        ...
        listContainer.innerHTML = sortedQuotas.map((quota, index) => `
            <div class="flex items-center justify-between py-4 px-4 border-b border-slate-custom/5 group transition-all bg-white/20 hover:bg-white/40 rounded-xl mb-3 hover-lift animate-fade-in-up table-row-animate flex-wrap gap-3" style="animation-delay: ${index * 0.05}s" data-id="${quota.id}">
                <div class="flex items-center gap-3 min-w-0 flex-1">
                    <div class="w-10 h-10 flex items-center justify-center border border-slate-custom/10 text-slate-custom/60 rounded-lg group-hover:border-primary/30 transition-colors flex-shrink-0">
                        <span class="text-xs font-bold">${quota.year.toString().slice(-2)}${quota.type.charAt(0).toUpperCase()}</span>
                    </div>
                    <div class="min-w-0 flex-1">
                        <p class="text-sm font-bold truncate max-w-[150px]">${quota.typeName || this.QUOTA_TYPES[quota.type]}</p>
                        <p class="text-xs text-slate-custom/40 truncate">有效期 ${quota.year + 1}.12.31</p>
                    </div>
                </div>
                <div class="flex items-center gap-3 flex-shrink-0">
                    ...
                    <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onclick="QuotaManager.editQuota('${quota.id}')" class="p-1.5 hover:bg-primary/10 rounded-lg transition-colors" title="编辑">
                            <span class="material-symbols-outlined text-base text-slate-custom/60 hover:text-primary">edit</span>
                        </button>
                        <button onclick="QuotaManager.deleteQuota('${quota.id}')" class="p-1.5 hover:bg-red-50 rounded-lg transition-colors" title="删除">
                            <span class="material-symbols-outlined text-base text-slate-custom/60 hover:text-red-500">delete</span>
                        </button>
                    </div>
                </div>
            </div>
